	...
	if pick['secondary_sequences_file_names']:
		...
		for m in possibleModalities:
			if m in prefixParts:
				...
				break
		if 'NIRCON' or 'VISCON' in pick['image_sequence_file_name']:
			indxVidNum = indxMod+2
		else:
		...
		for i in range(0,nSeq2):
			...
			for m in possibleModalities:
				if m in prefixParts:
					...
					break
			if 'NIRCON' or 'VISCON' in pick['image_sequence_file_name']:
				prefixParts[indxMod+2] = vidNum # plus 3 for quad
			else:
				prefixParts[indxMod+1] = vidNum # plus 3 for quad
